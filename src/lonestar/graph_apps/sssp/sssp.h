...
// ---
DEFINE_string(restoration_ctx_path, "lonestar/graph_apps/sssp", "Path to restoration context");
DEFINE_string(merged_kernel_file, "lonestar/graph_apps/sssp/device/merged.cl", "the path the mega kernel file");
DEFINE_string(persistent_kernel_file, "lonestar/graph_apps/sssp/device/standalone.cl", "the path the mega kernel file");
...
typedef struct {
	...
	cl_uint *noutgoing, *psrc, *edgessrcdst, *edgessrcwt;

} Graph;

cl::Buffer d_dist, d_in_wl, d_in_index, d_out_wl, d_out_index,  d_g_edgessrcdst, d_g_psrc, d_g_edgessrcwt;
cl_uint g_nnodes;
Graph hgraph;
cl_uint *zero_array;
cl_uint *read_array;
...
unsigned allocOnHost(Graph * g) {
  ...
  g->edgessrcwt = (cl_uint *)malloc((g->nedges + 1) * sizeof(cl_uint));
  return 0;
}
...
void init_persistent_app_for_occupancy(CL_Execution *exec) {
  ...
  d_g_edgessrcwt = cl::Buffer(exec->exec_context, CL_MEM_READ_WRITE, (hgraph.nedges + 1) * sizeof(cl_uint), nullptr, &err);
  check_ocl(err);
  ...
  err = exec->exec_queue.enqueueWriteBuffer(d_g_edgessrcwt, CL_TRUE, 0, (hgraph.nedges + 1) * sizeof(cl_uint), hgraph.edgessrcwt);
  check_ocl(err);
...
int set_persistent_app_args_for_occupancy(int arg_index, cl::Kernel k) {
  ...
  check_ocl(k.setArg(arg_index++, d_g_edgessrcwt));
  return arg_index;
}
