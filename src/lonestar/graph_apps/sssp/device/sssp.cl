  ...
  while (total_inputs-- > 0) {
    ...
    while (total_edges > 0) {
      ...
      if (get_local_id(0) < total_edges) {
        ...
        if (processedge2(dist, iteration, gather_offsets[get_local_id(0)], src[get_local_id(0)], &to_push, g_nnodes, g_edgessrcdst, g_psrc, g_edgessrcwt)) {
          ncnt = 1;
        }
      }
...
void drelax(__global uint *dist,
  ...
  if (iteration == 0) {
    ...
    processnode2(dist, in_wl, in_index, out_wl, out_index, gather_offsets, queue_index, scan_arr, loc_tmp, src, iteration, g_nnodes, g_edgessrcdst, g_psrc, g_edgessrcwt);  
  }
}
...
__kernel void drelax2(__global uint *dist,
  ...
  while (1) {
	  ...
	  drelax(dist, in_wl, in_index, out_wl, out_index, gather_offsets, queue_index, scan_arr, loc_tmp, src, iteration, g_nnodes, g_edgessrcdst, g_psrc, g_edgessrcwt);
