...
__kernel void
connect_four(__global uchar *base_board, const int maxlevel,
             ...
             __global IW_barrier *__bar, __global Discovery_ctx *__d_ctx,
             SCHEDULER_ARGS) {
  ...
  while (true) {
    ...
    int pool_id = p_get_group_id(__d_ctx) % num_task_pool;
    ...
    if (local_id == 0) {
      ...
      if (node->level >= maxlevel || value == PLUS_INF || value == MINUS_INF) {
        ...
        for (int i = 0; i < NUM_COL; i++) {
          int child_id = wgm_create_children(i, local_task[0], nodes, node_head,
                                             num_nodes);
