...
int scan_board(__local uchar *board, int row, int col, Direction direction) {
  ...
  } while (0 <= row && row < NUM_ROW && 0 <= col && col < NUM_COL);
...
void update_board(__local uchar *board, __global uchar *base_board,
                  __global Node *node, int local_id, int local_size) {
  ...
  if (local_id == 0) {
    ...
    for (int i = 0; i < node->level; i++) {
      ...
      while (row < NUM_ROW && board[(row * NUM_COL) + col] == EMPTY) {
        row++;
      }
...
int wgm_create_children(int move, int parent_id, __global Node *nodes,
                        __global atomic_int *node_head, int num_nodes) {
...
int wgm_task_pop(__global int *task_pool, __global atomic_int *task_pool_lock,
                 __global int *task_pool_head, const int task_pool_size,
                 int pool_id) {
  int task = NULL_TASK;
...
int wgm_task_push(int task, __global int *task_pool,
...
void wgm_update_parent(__global Node *nodes, int node_id,
...
void connect_four(__global uchar *base_board, const int maxlevel,
                  __global Node *nodes, __global atomic_int *node_head,
                  const int num_nodes, __global int *task_pool,
                  __global atomic_int *task_pool_lock,
                  __global int *task_pool_head, const int num_task_pool,
                  const int task_pool_size, __global int *next_move_value,
                  __global atomic_int *root_done,
                  // for debugging
                  __global atomic_int *debug_int, __global uchar *debug_board,
                  __local int *val, __local uchar *board,
                  __local int *local_task, __local _Bool *game_over,
  ...
  while (
    ...
    case 0:
      ...
      int pool_id = k_get_group_id(__k_ctx) % num_task_pool;
      ...
      if (local_id == 0) {
        ...
        for (int i = 0; i < num_task_pool; i++) {
          local_task[0] =
              wgm_task_pop(task_pool, task_pool_lock, task_pool_head,
                           task_pool_size, (pool_id + i) % num_task_pool);
      ...
      if (local_id == 0) {
        ...
        if (node->level >= maxlevel || value == PLUS_INF ||
            value == MINUS_INF) {
          ...
          for (int i = 0; i < NUM_COL; i++) {
            int child_id = wgm_create_children(i, local_task[0], nodes,
                                               node_head, num_nodes);
            int push_pool_id = pool_id;
            // loop on trying to push the task in a pool
            while (wgm_task_push(child_id, task_pool, task_pool_lock,
                                 task_pool_head, task_pool_size,
                                 push_pool_id) != NULL_TASK) {
...
mega_kernel(__global int *A, const int A_row, const int A_col, __global int *B,
            const int B_row, const int B_col, __global int *C,
            __global atomic_int *counter, __global atomic_int *hash,
            __global uchar *base_board, const int maxlevel,
            __global Node *nodes, __global atomic_int *node_head,
            const int num_nodes, __global int *task_pool,
            __global atomic_int *task_pool_lock, __global int *task_pool_head,
            const int num_task_pool, const int task_pool_size,
            __global int *next_move_value, __global atomic_int *root_done,
            // for debugging
            __global atomic_int *debug_int, __global uchar *debug_board,
...
#define PERSISTENT_KERNEL                                                      \
  connect_four(base_board, maxlevel, nodes, node_head, num_nodes, task_pool,   \
               task_pool_lock, task_pool_head, num_task_pool, task_pool_size,  \
               next_move_value, root_done, debug_int, debug_board, val, board, \
               local_task, game_over, bar, persistent_kernel_ctx, s_ctx,       \
