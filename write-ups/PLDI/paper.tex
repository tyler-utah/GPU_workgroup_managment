...
\HEComment{standard version: one task pool per workgroup, the number of
workgroup is a constant. So we access our pool by pool\_id ==
get\_group\_id(0). When we try to steal, we can look into other pools
which are indexed by (pool\_id + delta) modulo get\_num\_workgroups(0).
}

\begin{lstlisting}
__kernel connect_four(__global uchar *base_board,
                      const int lookahead,
                      __global atomic_int *best_move,
                      __global int *all_done,
                      __global Node *tree,
                      __global Task *pools,
                      __global Mutex *pool_m)
{
  // init
  if (get_global_id(0) == 0) {
    init_first_tasks(tree, pools);
  }
  global_barrier();

  // main loop
  while (true) {
    // obtain a task, or terminate if all work is done
    int pool_id = get_group_id(0);
    Task *t = pop_or_steal(pools, pool_m, pool_id);
    if (t == NULL) {
      if (*all_done) { return; } else { continue; }
    }

    // perform task, may create new tasks
    Node *node = tree[t->node_index];
    value = node_value(base_board, node);
    if (winning(value) || node->level == lookahead) {
      // we reached a leaf
      update_parent_value(node, tree, best_move, all_done);
    } else {
      // create_children() pushes or donates new tasks
      create_children(node, tree, pools, pool_m, pool_id);
    }
  }
}
\end{lstlisting}

\HEComment{ cooperative version: add kill/fork, and must pass the number
of task pools explicitely since get\_num\_workgroups() cannot be used to
know the number of pools. }
                      ...
                      const int num_pools)
{
  // init
  if (get_global_id(0) == 0) {
    init_first_tasks(tree, pools, num_pools);
  }
  global_barrier();
  ...
  while (true) {
    ...
    Task *t = pop_or_steal(pools, pool_m, pool_id, num_pools);
    if (t == NULL) {
      if (*all_done) { return; } else { continue; }
    }
    ...
    if (winning(value) || node->level == lookahead) {
      ...
      // create_children() pushes or donates new tasks
      create_children(node, tree, pools, pool_m, pool_id, num_pools);
    }
  }
}
\end{lstlisting}
...
design, e.g.~\cite{...}, and the atomic operations provided by the
OpenCL 2.0 memory model enable a memory-safe
...
lifetime of a cooperative kernel.  Although $\getnumgroups()$ has
atomic semantics, and so can be called at any time to yield a
well-defined value, the value returned by two successive calls may
differ if workgroups have executed $\offerkill$, $\offerfork$ or
$\resizingglobalbarrier$ in-between.  The programmer must therefore
write their cooperative kernel in a manner that is robust to changes
in the value returned by $\getnumgroups()$.
...
\section{The GPUs we Tested}

\ADComment{Describe the platforms.  Explain why we could not try other
  platforms.}

\section{Determining Representative Workloads}
...
\section{The Overhad of Moving to Cooperative Kernels}
...
\section{Performance and Responsiveness of Cooperative Scheduling}
...
\\
\forall j\;.\;w_0'(j) = (l_{0,j}, \globalbarrier(); \offerfork(); \globalbarrier(); \globalbarrier();\mathit{ss})
\\
\forall i \neq 0 \;.\;\forall j\;.\;w_i'(j) = (l_{i,j}, \globalbarrier(); \globalbarrier(); \offerkill(); \globalbarrier();\mathit{ss})
}
{
