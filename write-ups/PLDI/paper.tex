...
\item The model is backwards-compatible: if $\offerkill$ and
  $\offerfork$ are ignored, a cooperative blocking kernel will behave
  exactly as a regular blocking kernel does on current GPUs.
...
\item \emph{Cooperative kernels}, an extension to the GPU programming model to support the scheduling requirements of blocking algorithms (\mysec\ref{sec:cooperativekernels}). 
...
\item An experience report of porting \NumAlgorithms{} irregular GPU
  ...
  using three GPUs assessing the overhead associated with cooperative
  kernels, and the responsiveness of the multi-tasking that the
  approach enables (\mysec\ref{sec:experiments}).
...
task may create an arbitrary number of new tasks. Work stealing
enables dynamic balancing of tasks across several processing units,
OpenCL workgroups in our case. Each workgroup has an associated task
queue from which it obtains tasks to process, and to which it stores
newly created tasks. When a workgroup's queue becomes empty, the
workgroup---rather than terminating---tries to \emph{steal} a task
from an other queue. Workgroups keep on processing tasks until all the
work is done. Previous work addresses the feasibility and efficiency
of work stealing on GPUs~ref\cite{dlb-web,TPO10}.
...
queues, which reside in global memory, and which were initialized by the
...
the number of active workgroups $M$ being atomically decremented by one.  Case
...
The $\offerfork$ primitive (also a workgroup-level function) allows the cooperative kernel to indicate
...
integer $k$ in the range $[0, N-M]$ is chosen by the scheduler;
...
\subsection{Sizing Non-Persistent Workloads}\label{sec:representativeworkloads}

\HEComment{given the paper title, maybe ``non-blocking'' would be more
  appropriate than ``non-persistent''?}

Cooperative kernels enables other \emph{non-persistent} workloads to
access GPU resources without stopping the blocking algorithm
computation entirely. We size these workloads to represent
approximatively the workload required by a system to maintain smooth
screen animations.

\paragraph{Experimental setup} \ADComment{Describe the data we are
  gathering.}  We repetitively run a kernel with a controlled workload
and detect from which workload size the screen animation starts to
jitter. Given the kernel workload and its scheduling frequency, we
deduce what is the workload and frequency needed by the system to
maintain a smooth animation. We use this approach to measure required
by the system to maintain smooth animations of three types:
(\emph{light}) emphasis of desktop icons under the mouse pointer,
(\emph{medium}) window dragging over the desktop, and (\emph{heavy})
WebGL animations in a browser.
...
heavy & 10 & 40\\
\end{tabular}
\caption{Caracteristics of non-persitent workloads}
\label{tab:nonpersistentworkload}
\end{table}

\paragraph{Results} \ADComment{Discuss the results.}
\mytab~\ref{tab:nonpersistentworkload} illustrates the caracteristics
...
of seconds which coincide with the human persistence of vision. The
...
\paragraph{Experimental setup} \ADComment{Describe the data we are
  gathering.}  The introduction of cooperative scheduling primitives
introduces an overhead even if no killing, forking or resizing ever
happen. We measure this overhead by comparing execution times of the
original kernel and the cooperative one, forcing the scheduler to
never modify the number of active workgroups. Since the cooperative
kernel contains the non-persistent task code anyway, its occupancy
bound may be less than the original kernel; we make sure to use this
occupancy bound as the number of workgroups for the original kernel in
order to compare relevant execution times.

\HEComment{Also include this occupancy reduction as an overhead?}
...
Python\footnote{https://docs.python.org/3/reference/datamodel.html#coroutine-objects}}
or Lua\footnote{https://www.lua.org/pil/9.1.html}, or Ruby's fiber
\footnote{\url{http://ruby-doc.org/core-2.1.1/Fiber.html}}. In the
context of GPU resources, cooperative multitasking seems to refer to
sizing kernels such that they do not compute for too long in order to
yield the whole GPU regularly~\cite{adriaens2012case}. Our
