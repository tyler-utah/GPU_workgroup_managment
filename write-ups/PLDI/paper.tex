...
screen animations. We repetitively ran a kernel with a controlled
workload and detected from which workload size the screen animation
started to jitter. Given the kernel workload and its scheduling
frequency, we can infer what is the workload and frequency needed by
the system to maintain a smooth animation.

Based on this approach, we measured workloads required by the system
to maintain smooth animations of three types: (\emph{light}) emphasis
of desktop icons under the mouse pointer, (\emph{medium}) window
dragging over the desktop, and (\emph{heavy}) WebGL animations in a
browser. \mytab~\ref{tab:nonpersistentworkload} illustrates our
results. In order to simulate such non-persistent workloads alongside
kernels of blocking algorithms, we use a matrix multiplication kernel
where matrix dimensions are tuned to trigger the relevant execution
duration.
