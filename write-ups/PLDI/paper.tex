...
$\mathsf{process\_task}$ function.  Processing this task may lead to the creation of further
...
\ref{line:cgraph:rechunking2}).  \ADComment{Tyler: can you please add the line number references in the following?} This example exploits the fact that the cooperative kernel does not issue $\offerkill$ nor $\offerfork$ directly: the value of $\keyword{stride}$ obtained from $\getglobalsize()$ at line~\ref{...} is stable until the next resizing barrier at line~\ref{...}, and in particular can be safely used inside the $\keyword{for}$ loop.
...
achieve a smooth frame rate, and we evalaute responsiveness in
...
accept an $\offerkill$ only if a comput unit is required for another
...
\ADComment{TODO: Discuss downside, which is that if WG with high id is
  doing a chunky task, a WG with a lower id may be ready to be killed
  but cannot be taken.  Work stealing algorithms, which do not depend
  on thread id, could take advantage of this.}


\paragraph{Keeping one workgroup alive}

\ADComment{TODO: To simplify programming, avoiding the need to wrap all kills in conditionals, we always keep one workgroup alive.  This fits well with our decision to kill WGs in decreasing order.}

\ADComment{TODO: There are possible use cases for allowing all WGs to be killed, though, and we might wish to allow them.}


\paragraph{Transmission of partial state from a single thread}

\ADComment{TODO: explain why we have thread 0 transmit its state, and transmitting only annotated variables.}


\ADComment{Something old to maybe work in:}
