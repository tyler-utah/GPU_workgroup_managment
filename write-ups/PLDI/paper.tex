...
\myfig~\ref{fig:} reports the overhead brought by the scheduling of
non-cooperative kernels over the cooperative kernel execution time
(geometric mean over all applications and inputs on the Iris chip, other
chips look similar\HEComment{We can put the graphs of ther chips in
appendix}), along with the period (aritmetic mean over all applications
and inputs) at which non-cooperative kernels can be scheduled. We show
...
Non-cooperative kernels running a workload with a single workgroup leads
to almost no overhead, but the period is far too high to meet the needs
...
averages \~10\% over its goal period (mean of 44 ms). If asking for all
but one, the heavy workload can reach its goal period.

Yet, as expected, allocating more non-cooperative workgroups increases
the execution time overhead of the cooperative kernel. Still the heavy
workloads meet their period (by allocating all but one workgroup for
non-cooperative kernels) for an overhead less than 1.4 on average. Light
and medium workloads meet their period with only a negligable overhead
of the cooperative kernel (less than 1.03 slowdown).
