...
\paragraph{$\offerkill$ implementation}
...
graph algorithms, based on Pannotia~\cite{...} and Lonestar~\cite{...}
...
sequence. Responsiveness, i.e., how fast can a non-cooperative kernel
can be scheduled, depends not only on the scheduler implementation, but
also on how often the cooperative kernel offer workgroups to be killed.
...
duration when this non-cooperative kernel have access to all available
workgroups. Besides, we have different run where the non-cooperative
kernel is scheduled with different number of workgroups: either 1, a
quarter, a half or all but one workgroups among the ones available for
the cooperative kernel. Moreover, in graph algorithms we try both the
regular and the query barrier.

Our experiments span 13 pairs of cooperative kernels and data sets, 3
classes for the non-cooperative kernel, 4 quantities of workgroups
claimed for the non-cooperative kernel and 2 variations of resizing
barriers for graph algorithms, leading to 288 configurations.

A run is characterized by the cooperative kernel, its data set, the
non-cooperative kernel, its size, the number of workgroups we ask for
the non-cooperative kernel. This forms a configuration.

For each GPU, we first 
...
run cooperative kernels and fire non-cooperative kernels regularly,
asking for different number of workgroups

explain gather time, execution time. related to responsiveness and
performance

explain query barrier, or mention the results with / without query
barriers.

recall 10 runs for each configurations. How many configurations ?

Mention that some configuration failed.

We check correctness (completion) of both kernels. Some errors
sometimes.
...
\paragraph{Dynamic parallelism} Recent version of CUDA and OpenCL allow a kernel to spawn further kernels~\cite{...}.
This \emph{dynamic parallelism} can be exploited to implement a GPU-based scheduler: an initial scheduler kernel uses the feature to spawn additional kernels as required~\cite{ }.
...
\bot}_{N-M}))\]
%
where: $\sigma$ represents the shared state of the kernel; $M
\leq N$ is the number of \emph{active} workgroups; $w_i$ is the
workgroup state for active workgroup $i$ ($0 \leq i < M$); and $N-M$
occurrences of $\bot$ indicate \emph{absent} workgroups.  Let $\sharedstates$ denote the set of all possible shared states.
...
GPUs are known to have relaxed memory models~\cite{ }.  To abstractly
