...
\getlocalsize() + \getlocalid()$.

\ADComment{Maybe cut?} Within a workgroup, threads may also be
organised into \emph{subgroups} (termed \emph{warps} and
\emph{wavefronts} on NVIDIA and AMD architectures, respectively).
...
application.  Global memory is shared among all threads executing a
...
operations, based on the C11 memory model~\cite{...}.  In particular,
...
kernel that uses a work stealing queue.
%
...
pool. Hence, workgroups keep on processing tasks until all the work is
done.

\myfiglong\ref{fig:workstealing} illustrates an OpenCL implementation
of a work stealing algorithm. All threads start their execution at the
...
workgroup, although inside only the thread with a local id equal to
...
participate in the processing of the task inside the
...
mutex which is implemented using atomic compare and swap operations on
...
is not provided by current GPU programming models.  In
Section~\ref{sec:cooperativeexamples} we show how the example can be
updated to use our cooperative threads programming model to resolve
