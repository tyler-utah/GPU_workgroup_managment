...
\subsection{The megakernel approach}\label{sec:megakernel}
...
The scheduler context provides an \emph{assignment mutes} to protecting concurrent calls to
$\offerfork$ and $\offerkill$.  On calling $\offerkill$, a workgroup is only permitted to exit if the scheduler currently holds the assignment mutex; that is, the scheduler is trying to obtain work
groups to send to a task.  Conversely, on calling $\offerfork$ a workgroup can only spawn new workgroups if the assignment mutex is not held by the scheduler.

Concurrent calls to $\offerkill$ are acceptable because only the workgroup with the highest id may exit, automatically ensuring mutual exclusion for exit-related modifications.
...
workgroups from the cooperative kernel.

On the middle and right figures, which depict graph algorithms, we find
again the same trend: as the number of workgroups allocated to the
non-cooperative task grows, the non-cooperative kernel execution time
lowers and the gather time raises. When the regular resizing barrier is
used, gather time can become quite long and exceed the execution time.
However, the scheduling responsiveness can benefits from the query
barrier: when this barrier is used, the gather time becomes almost
stable w.r.t. the number of workgroups that needs to be collected, since
the scheduler collects all needed workgroups as soon as they hit a
resizing barrier.
