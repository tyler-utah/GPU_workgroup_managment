...
$\keyword{goto}$ to direct threads joining the cooperative kernel to
the appropriate location, and $\keyword{return}$ to force threads to
leave the computation.  These restrictions still allow us to
experiment with interesting irregular algorithms (see
\mysec\ref{sec:experiments}), and could all be removed with
engineering effort, but this effort might not be well invested since a
full implementation of coopeartive kernels would not use the
megakernel approach we present here.
...
maintain consistent context states for each kernel, e.g.\ the number
of active workgroups computing a cooperative kernel, and (c) provide a
safe way for these states to be modified in response to $\offerfork$ and $\offerkill$. We now discuss these issues in more detail.
...
the OpenCL primitives $\getnumgroups()$ and $\getgroupid()$ provide this functionality natively. For multiple
kernel execution, we emulate these primitives via a per kernel \emph{context}, containing, for a given kernel: the number of active groups $M$, the total number of requested groups $N$, and a mapping from the native workgroup id to a new workgroup
id valid for the kernel.  In the case of a non-cooperative kernel, $M$ is undefined because the notion of active workgroups is not relevant.
...
provide a channel of communication from the scheduler to
...
kernels a \emph{resource message}, commanding workgroups executing
the kernel to exit at $\offerkill$, or 
...
consists of: a kernel to execute, the inputs to the kernel, the
...
On receiving a data packet from the host
...
kernel context $\KC$.  Note that if the new kernel $K$ is itself a cooperative kernel, the scheduler could decide to provide $K$ with fewer than $N$ active workgroups initially.

If a cooperative kernel $K_c$ is executing with fewer workgroups than it initially requested, the scheduler may decide to give $K_c$
additional workgroups if $K_c$ calls $\offerfork()$. To do this, the scheduler asynchronously signals $K_c$ through $K_c$'s
...
\paragraph{Safe implementation}

Concurrent calls to $\offerfork()$ must be protected with a mutex,
otherwise workgroups calling $\offerfork()$ will conflict over the
...
calling $\offerfork()$ occurs between the scheduler assigning tasks.
Thus we provide a mutex, called the \emph{assignment mutex}, in the
scheduler context which must be held anytime a group of available
resources is assigned to a kernel.

The provides an indirect mutex protecting concurrent calls to
$\offerfork()$ and $\offerkill()$. That is, $\offerkill()$ only
commands a workgroup to exit when the scheduler holds the
\emph{assignment lock}, as the scheduler is trying to obtain work
groups to send to a task, and $\offerfork()$ cannot spawn workgroups
without holding this lock.

The semantics of $\offerkill()$, which allow only the workgroup with
the highest id provide natural mutual exclusion for exiting workgroups
(as part of the exit, the kernel context is updated). That is, a
workgroup can only exit at $\offerkill()$ if it is the workgroup with
the highest id. An exiting thread releases this natural mutex by
decrementing the number of active workgroups in the kernel context.
...
In section~\ref{??}, we show that a resizing barrier is semantically
equivalent to three normal barrier calls with calls to cfork and ckill
in between. It is possible, however to implement the resizing with
...
(e.g. \cite{??}). That is, one workgroup (the master) collects signals
...
needs them to exit. We show in Section~\ref{} that the barrier using
...
programming style for long-running kernels~\cite{owens-persistent,DBLP:conf/ipps/KaleemVPHP16,DBLP:conf/ipps/DavidsonBGO14,DBLP:conf/hipc/HarishN07,DBLP:journals/topc/MerrillGG15,DBLP:conf/egh/VineetHPN09,DBLP:conf/ppopp/NobariCKB12,DBLP:conf/hpcc/SolomonTT10a,DBLP:conf/popl/PrabhuRMH11,DBLP:conf/ppopp/Mendez-LojoBP12,DBLP:conf/oopsla/PaiP16,DBLP:conf/oopsla/SorensenDBGR16,DBLP:conf/egh/CedermanT08,TPO10,BNP12,Pannotia}.  These
