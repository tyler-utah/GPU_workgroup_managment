...
\ADComment{Describe the data we are gathering.}

Cooperative scheduling primitives entails an overhead even if no
killing, forking or resizing ever happen. We measure this overhead via
the execution time ratio between the original kernel and the
...
non-persistent task code anyway, its occupancy bound may be less than
the original kernel; we make sure to use this occupancy bound as the
number of workgroups for the original kernel in order to compare
relevant execution times. \HEComment{Do we need to mention the
  possible occupancy reduction as an overhead?}

\paragraph{Results} \ADComment{Discuss the results.}
\mytab~\ref{tab:overhead} lists overhead slowdown ratios, both
geometric mean and maximum value. The overhead is below a 2 times
slowdown even in the worst case. We consider these results as
acceptable, given that our SVM-based scheduler performance is likely a
far upper bound of what a scheduler implemented directly in the
runtime could offer.
