...
We now assess the performance impact and responsiveness of
multitasking between a long-running cooperative kernel and a series of
short, non-cooperative kernel launches.  Performance is measured by
comparing the time for multitasked execution with the time taken to
perform all kernel launches in sequence.  Responsiveness, i.e.\ how
...
kernel offers workgroups to be killed.
...
\ADComment{Not clear: are the times for light and heavy averages?} A logarithmic
...
for non-cooperative kernels.  These graphs show a representative sample of our results; the full set of graphs for all configurations is provided in Appendix~\ref{appendix:extraresponsiveness} (supplementary material).
...
other two GPUs.  \ADComment{Added this; is it right?}  These
algorithms use barriers, and we experimented with the regular and
query barrier implementations described in \mysec\ref{sec:resizingbarrier}.  The
results shown are for executions that use the more efficient query barrier, except that we also show the gather time associated with the regular barrier for comparison.  The graphs show a similar trend to the left-most graph: as the number of non-cooperative workgroups grows, the
...
\paragraph{Performance} \ADComment{I don't think it's clear how overhead is computed; we need to make it clear that it is a slow-down.  Let's discuss.}
\myfiglong\ref{fig:performance} reports, for the Iris GPU, the overhead brought by the scheduling of
non-cooperative kernels over the cooperative kernel execution time (geometric mean over all applications and inputs), along with the period (aritmetic mean over all applications
and inputs) at which non-cooperative kernels can be scheduled.
The results for the other GPUs are similar and are graphed in Appendix~\ref{appendix:extraperformance} (supplementary material).
 We show
...
\section{Graphs for Multitasking Experiments}

In \mysec\ref{sec:responsiveness} we showed a selection of graphs to illustrate our
experimental results related to performance and responsiveness.  Here
we present the full set of graphs.

\subsection{Responsiveness graphs}\label{appendix:extraresponsiveness}
...
\ADComment{Tyler: show all the graphs, perhaps grouped by GPU.}

\subsection{Performance graphs}\label{appendix:extraperformance}

\myfiglong\ref{fig:performance} presents data on the performance impact and period associated with multitasking cooperative and non-cooperative workloads, for the Iris GPU.

\ADComment{Tyler: say something like ``Corresponding graphs for the XXX and YYY GPUs are shown in \myfigs\ref{ } and~\ref{ }, respectively.'', adding the graphs.}
