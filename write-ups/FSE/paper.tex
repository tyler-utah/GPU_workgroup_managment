...
this, Nvidia's most recent compute capablity does not guarantee
...
In return, the scheduler should allow the cooperative kernel to utilise
hardware resources where possible, killing workgroups only when demanded by other tasks, and forking additional workgroups when possible.
...
task. The workgroups are subsequently restored to the cooperative kernel when workgroup 0 calls $\offerfork$. The order in
which workgroups are killed is important and discussed in
\mysec{\ref{sec:semantics}}. The \emph{gather time} is the time
...
\myfiglong\ref{fig:workstealing} illustrates a work
stealing kernel. Each thread receives a pointer to the
task queues, in global memory, initialized by the
host to contain the initial tasks. A thread uses its workgroup id 
(line~\ref{line:wksteal:getgroupid}) as a queue id to access the relevant task queue. The
...
being created and pushed to the workgroup's queue.
...
%
In Supplementary material we present a formal semantics for
cooperative kernels atop an abstract GPU programming model, and
discuss several cases where we might have taken different and also
reasonable semantic decisions.
...
workgroup has the highest largest id of all active workgroups then it
can be killed by the scheduler, except that workgroup 0 can never be
killed (to ensure that the kernel computation does not terminate
early).  More formally, if $m < M-1$ or $M=1$ then $\offerkill$ is a
...
%\vspace{-2mm}
\caption{Example gather time and non-cooperative timing results \TSComment{need to get updated numbers}}\label{fig:fine-grained-timing}
\vspace{-3mm}
\end{figure*}
...
\hline
$1.09$ & $1.23^{\ddagger}$ & $1.09$ & $1.21^{\diamond}$ & $1.11$ & $1.23^{\ddagger}$ \\
\end{tabular}\\
{\small
 $^{\ddagger}$octree, $^{\diamond}$sssp usa\_ny
}
%\vspace{-2mm}
\caption{Cooperative kernel slowdown w/o multitasking \TSComment{need to get updated numbers}}
\label{tab:overhead}
\vspace{-8mm}
\end{table}
...
%\vspace{-2mm}
\caption{Performance impact of multitasking cooperative and non-cooperative workloads, and the period with which non-cooperative kernels execute \TSComment{need to get updated numbers}}\label{fig:performance}
\vspace{-5mm}
\end{figure}
...
allocated to the non-cooperative kernel, the heavy workload averages
$\sim$10\% over its goal period (mean of 44 ms). If all-but-one are
allocated, the heavy workload reaches its goal period.
%
...
\subsection{Comparison with Task Level Preemption}\label{sec:nvidiacomparison}
...
\hline
light & 1.04 & 1.XX($N/4$)\\
medium & 1.08 & 1.XX($N/4$)\\
heavy & 1.33 & 1.XX($N-1$)\\
\end{tabular} \\
%\vspace{.2cm}

\caption{Overhead of kernel level preemption vs cooperative kernels for three
graphics workloads \TSComment{need to get updated numbers}}
...
this works are available, but from reading Nvidia documentation
and online material~\cite{PascalWhitepaper,anandtech}, and running
litmus tests on a Pascal platform, our understanding is that
preemption of entire kernels, but not of individual workgroups running
a kernel is supported.  Intel GPUs do
not provide this feature, and out OpenCL 2.0 prototype of cooperative kernels cannot run on Nvidia GPUs, making a direct comparison impossible.
We present here a theoretical analysis of the overheads associated with
sharing the GPU between graphics and compute tasks via kernel-level
preemption.

Let us assume that a graphics workload is required to be scheduled
with period $P$ and duration $D$, and that a long-running compute
kernel requires time $C$ to execute without interruption.  If we
assume the cost of preemption is negligible (e.g.\ Nvidia have
reported preemption times of 0.1 ms for Pascal~\cite{TODO}), then the overhead
associated with switching between compute and graphics every $P$ time
steps is $P/(P-D)$.

We compare this task-level preemption overhead
model with our experimental results per graphics workload in
\mytab{\ref{tab:preemption}}.
%
Based on the above assumptions, our approach provides lower overhead for low and medium
graphics workloads. Recall for the heavy graphics workload, we are
within $10\%$ of the required deadline ($40$ ms) by using half the
workgroups with an overhead of $1.XX$, which is less than the
kernel-level preemption overhead of $1.33$. \TSComment{need to get
  updated numbers} \ADComment{I don't quite understand the wording; sounds a bit like we still win with the heavy workload.}

The low overhead of cooperative kernels for low and medium graphics
workloads is due to sub-linear scalability of the irregular algorithms that they implement: some resources can
be taken away during computation without proportionally reducing
execution time.  Our poor performance for the heavy workload is
because the graphics task requires all but one workgroup, which
cripples the cooperative kernel enough that $\offerfork$ calls are not
issued as frequently.

These results suggest that a hybrid preemption scheme may work the
best for multi-tasking and graphics. That is, the cooperative approach
works well for light and medium tasks, but heavy graphics tasks
...
preemption, though, as discussted in \mysec{sec:intro} and
\mysec{sec:nvidiacomparison}, it is not clear whether they guarantee
