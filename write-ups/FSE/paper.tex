...
\ADComment{Update intro to be clearer that even with preemption our approach has merit, and that preemption does not imply fairness.}

\ADComment{Use NVIDIA/Pascal as motivation for our approach.}
...
\ADComment{Tyler: briefly state that this still occurs on Pascal, despite it having support for preemption.}
\TSComment{I added this but actually think it fits better a little bit later (marked with a comment). See what you think}
...
cases.
%
\TSComment{added note about Pascal experiment here. I wonder if there's
a better way to glue these paragraphs together? That is, above it provides
two issues vendors have with forward progress and below, we show a concrete
example of both issues in Nvidia's latest chips}
%
...
documentation still does not provide concrete forward progress
properties (i.e. vendors will not yet commit to providing these
properties). Furthermore, we were able to run some small experiments
on a Pascal GPU (GTX Titan X) and observed that the GPU still adhered
to the occupancy-bound execution model. That is, a kernel containing a
global barrier executed successfully when run with 56 workgroups, but
deadlocked when run with 57 workgroups. This is an example of a vendor
supporting preemption, yet not providing full fairness properties.
...
\subsection{Preemption on NVIDIA Comparison}\label{sec:nvidiacomparison}

\ADComment{Make the point that NVIDIA GPUs now support
  instruction-level preemption in hardware; allows arbitrary
  preemption.}

\ADComment{Note that they are doing coarse-grained, task-level
  preemption, rather than running multiple tasks concurrently on the
  GPU.}

\ADComment{Direct comparison is not possible as we cannot implement
  cooperative kernels on NVIDIA devices due to lack of mature OpenCL
  2.0 support.}

\ADComment{However, based on results reported by NVIDIA and a clock
  speed calculation we can estimate...}
