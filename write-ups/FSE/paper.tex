...
\caption{Cooperative kernels can flexibly resize to let run other tasks
(e.g. graphics) concurrently}
\label{fig:overview}
\end{figure}
...
\myfiglong{\ref{fig:overview}} shows an example execution of how the
GPU compute units might be shared between a cooperative kernel and a
graphics task. Notice that workgroups executing the cooperative kernel
are killed at a $\offerkill$ command to make room for a graphics
task. After the graphics task, the previously killed workgroups are
restored to executing the cooperative kernel when a workgroup
executing the cooperative kernel calls $\offerfork$. The order in
which workgroups are killed is important and discussed in
\mysec{\ref{sec:semantics}}. The \emph{gather time} is the time
between when resources are requested and when the application has
surrendered the resources via $\offerkill$. To satisfy soft-real time
...
this works are available, however from reading NVIDIA documentation
...
a kenrel is supported.  Intel---the only vendor whose OpenCL
implemenation is capable enough to implement cooperative kernels---do
not provide this feature, thus we cannot make a direct comparison.  We
present here a theoretical analysis of the overheads associated with
...
reported preemption times of 0.1 ms for Pascal), then the overhead
associated with switching between compute and graphics every $P$ time
steps is $P/(P-D)$. We compare this task-level preemption overhead
model with our experimental results per graphics workload in
\mytab{\ref{tab:preemption}}. 

We see that our approach provides lower overhead for low and medium
...
workloads are due to the irregularity of the cooperative kernels and
the property that they do not scale linearly. Thus some resources can
