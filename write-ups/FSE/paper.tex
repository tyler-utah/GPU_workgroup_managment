...
and inputs). We use the median as our data included some drastic
outliers; these outliers appear to happen with applications/inputs in
which the resizing barrier are not called very frequently and the
graphics task asks for half or more resources. For example, a medium
graphics workload for bfs on the rmat input has over an 8$\times$
overhead, while the same application on the usa input has an overhead
of 1.11$\times$. As \myfiglong\ref{fig:performance} shows, most of our
applications/inputs are much better behaved than this. Future work
includes examining these problem applications/inputs in more detail,
possibly to insert more calls to $\offerfork$ and $\offerkill$.
...
reported preemption times of 0.1 ms for Pascal~\cite{anandtech}), then
the overhead associated with switching between compute and graphics
...
configuration that allowed us to meet to deadline of the graphics
task.
%
Based on the above assumptions, our approach provides similar overhead
for low and medium graphics workloads; however, for the high workload,
our approach has a higher overhead.
...
well for multi-tasking and graphics. That is, the cooperative approach
